...
class RENet(torch.nn.Module):
    ...
    def pre_transform(seq_len: int) -> Callable:
        class PreTransform:
            ...
            def get_history(self, hist: List[int], node: int, rel: int) -> Tuple[Tensor, Tensor]:
                hists, ts = [], []
