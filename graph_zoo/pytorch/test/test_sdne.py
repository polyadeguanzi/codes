...
from pytorch.models.sdne import SDNE
from pytorch.utils import dataset_sdne as dataset
def parse_args():
    parser = ArgumentParser(formatter_class=ArgumentDefaultsHelpFormatter,
                            conflict_handler='resolve')
    parser.add_argument('--input', default='/mae_pat/xi.sun/SDNE-based-on-Pytorch/data/cora/cora_edgelist.txt',
                        help='Input graph file')
    parser.add_argument('--output', default='/mae_pat/xi.sun/SDNE-based-on-Pytorch/dat/cora/Vec.emb',
...
if __name__ == '__main__':
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for index in Data:
            ...
            opt.zero_grad()
            L_1st, L_2nd, L_all = model(adj_batch, adj_mat, b_mat)#.to(device)
            L_reg = 0
    ...
    assert embedding.isnan().sum()==0
    assert embedding.isinf().sum()==0
    outVec = embedding.detach().cpu().numpy()
